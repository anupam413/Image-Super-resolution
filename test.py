...
def process(input_image_path,scale):
    # scale = 4

    # load the model from model
    if scale == 4:
        model = torch.load('./final/checkpoint_x4/None_583000.pth.tar', map_location='cpu')
        net = FENet.FENet(multi_scale = False, group=4,scale=4 )
        net.load_state_dict(model['model_state_dict'])
    elif scale == 2:
        model = torch.load('./final/checkpoint_x2/None_594000.pth.tar', map_location='cpu')
    ...
    # output_image.save('./final/output_ada.jpg')
