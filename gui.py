...
def show_sr(path, scale=2):
    ...
    toolbar = NavigationToolbar2Tk(canvas, popup_sr)
    toolbar.update()
    canvas._tkcanvas.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
...
tk.Tk.wm_title(root, "Super Resolution")
label = ttk.Label(root, text="Super Resolution using Deep Learning", font=("", 22, "bold"))
label.pack(side="top", pady=30, padx=50)

desc = '''This GUI allows people to use the Super Resolution Model with ease.
All you need to do is to select the image, and the rest would be managed by the GUI.'''
...
label = ttk.Label(root, justify=tk.CENTER,
                  text="Select the image", font=("Verdana", 11))
